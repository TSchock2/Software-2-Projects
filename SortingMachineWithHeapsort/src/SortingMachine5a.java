...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last, Comparator<T> order) {
        ...
        /*
         * If array is equal to length 1, it is a heap. If the length is greater
         * than one, check to make sure array is already a heap. If not, sift
         * the value down to make it a heap.
         */
        if (array.length > 1) {

            // Make smallest the top index to compare to left and right
            int smallest = top;

            /*
             * If left index is less than or equal to last index and the value
             * at left index in array is less than or equal to the value at
             * smallest index(top in this case), left is the new smallest index.
             */
            ...
            /*
             * If right index is less than or equal to last index and the value
             * at right index in array is less than or equal to the value at
             * smallest index(top or left), right is the new smallest index.
             */
            ...
            /*
             * Compare the value at top to the value at smallest. If the value
             * at top is greater than the value at smallest, exchange the
             * entries and sift it down the heap to make it a complete binary
             * tree.
             */
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        // If array length is greater than 1, heapify subtrees of given top
        if (array.length > 1) {

            /*
             * If the left child is less than or equal to the last node, heapify
             * the left subtree to make it a complete binary subtree
             */
            if (left <= last) {
            ...
            /*
             * If the right child is less than or equal to the last node,
             * heapify the right subtree to make it a complete binary subtree
             */
            if (right <= last) {
        ...
        // SiftDown the subtree to heapify the subtree in increasing order down the tree
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // Add each value into the heap from the queue
        for (int i = 0; i < heap.length; i++) {
            heap[i] = q.dequeue();
        }

        // Make it a complete binary tree starting from index 0
    ...
    public final void changeToExtractionMode() {
        ...
        // InsertionMode changed to false prohibits any more entries added.
        this.insertionMode = false;

        // Create Heap for manipulation
    ...
    public final T removeFirst() {
        ...
        // The first value in the heap
        T first = this.heap[0];

        /*
         * If the heap size is greater than one, values must be exchanged,
         * heapSize decremented, and sifted down to keep complete binary tree.
         */
        if (this.heapSize > 1) {
            ...
            // Only decrement heapSize if Size one
            this.heapSize--;
        }

        assert this.conventionHolds();
        // Returns first values in heap
        return first;
    }
    ...
    public final int size() {
        ...
        // When in Insertion mode, the length of the queue is returned
        int size = this.entries.length();

        // When in extraction mode, the length of the heap is returned
        if (!this.insertionMode) {
            size = this.heapSize;
        }
        ...
        // Returns length based on which mode it is in
        return size;
    }
